...
const ORDER_API              = 'http://localhost:3006';

// Stripe is initialised once at module level — never inside a component
const stripePromise = loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY);

// ─── Helpers ──────────────────────────────────────────────────────────────────

/**
 * Build a deterministic UUID-shaped string from a numeric/string user ID.
 * This avoids generating a random UUID on every call, which caused the same
 * user to get a different ID every time handleOrderSubmit ran.
 */
...
/** Strip undefined values from an object (keeps the payload clean for the API). */
const stripUndefined = (obj) =>
  Object.fromEntries(Object.entries(obj).filter(([, v]) => v !== undefined && v !== ''));

/** Calculate order totals from cart summary + shipping method. */
...
export default function CheckoutPage() {
  ...
  // Run this once on mount / when deps change — not on every render
  useEffect(() => {
    if (!user) {
  ...
  }, [user]); // only re-run if user object changes, not on every render

  // ── Totals — memoised so they don't recalculate on unrelated renders ───────
  ...
  const handleOrderSubmit = useCallback(async () => {
    ...
    // Validate all cart items have a valid price before hitting the API
    ...
    try {
      // Health check — give a clear error if the order service is down
      // Uses AbortController so it times out quickly rather than hanging
      ...
      // Clear cart — non-critical, don't let it block confirmation
      try {
        await clearCart();
      } catch (e) {
      ...
      // Persist for the confirmation screen
  ...
  }, [user, cart, orderData, clearCart, toast, router]);
  ...
  // ── Loading guard — shown while auth/cart state is still resolving ─────────
  if (!user || !cart) {
    return (
