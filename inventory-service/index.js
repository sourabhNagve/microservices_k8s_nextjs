...
// FIX 1 (original: none): no env-var validation. JWT_SECRET is required
// because the auth middleware verifies tokens on every route.
const requiredEnvVars = ['JWT_SECRET', 'INVENTORY_DATABASE_URL'];
...
const app  = express();
const PORT = parseInt(process.env.INVENTORY_SERVICE_PORT, 10) || 3005;

// ─── Security middleware ──────────────────────────────────────────────────────
app.use(helmet({ crossOriginEmbedderPolicy: false }));

// FIX 2 (original): dynamic origin validation with whitespace trim.
...
// ─── Rate limiting ────────────────────────────────────────────────────────────
// FIX 3: standardHeaders/legacyHeaders; body limit reduced from 10mb to 1mb.
...
const startServer = async () => {
  try {
    ...
    const shutdown = (signal) => {
      console.log(`\n${signal} — shutting down inventory service...`);
